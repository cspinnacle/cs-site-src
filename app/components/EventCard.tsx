...
export default function EventCard({ event, index }: { event: EventItem; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="flex gap-5 bg-surface p-5 shadow-[0_4px_24px_-8px_rgba(22,35,61,0.10)] hover:-translate-y-1 hover:shadow-[0_20px_40px_-12px_rgba(22,35,61,0.16)] transition-all"
    >
      <div className="text-center shrink-0 min-w-[56px]">
        <div className="text-2xl font-bold text-heading leading-none">{day}</div>
        <div className="text-xs text-comment uppercase tracking-wide mt-1">{month}</div>
      </div>
      <div>
        <h3 className="text-base font-semibold text-heading mb-1">{event.title}</h3>
